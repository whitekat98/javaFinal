...
//        JsonStream.setMode(EncodingMode.DYNAMIC_MODE);
    ...
    public void test_enum() throws IOException {
        ...
        stream.close();
        assertEquals("{'field1':'HELLO'}".replace('\'', '"'), baos.toString());
        Config cfg = new Config.ReBuilder()
    ...
    public void test_not_nullable() {
        ...
        Config config = new Config.ReBuilder()
                .encodingMode(EncodingMode.DYNAMIC_MODE)
                .build();
    ...
    public void test_indention() {
        Config dynamicCfg = new Config.ReBuilder()
                .indentionStep(2)
                .encodingMode(EncodingMode.DYNAMIC_MODE)
                .build();
        ...
        assertEquals("{\n" +
                ...
                "  \"field3\": null\n" +
                "}", output);
        Config reflectionCfg = new Config.ReBuilder()
                .indentionStep(2)
    ...
    public void test_indention_with_empty_object() {
        Config config = JsoniterSpi.getCurrentConfig().copyReBuilder()
                .indentionStep(2)
                .encodingMode(EncodingMode.REFLECTION_MODE)
                .build();
        assertEquals("{}", JsonStream.serialize(config, new TestObject15()));
        config = JsoniterSpi.getCurrentConfig().copyReBuilder()
                .indentionStep(2)
                .encodingMode(EncodingMode.DYNAMIC_MODE)
                .build();
        assertEquals("{}", JsonStream.serialize(config, new TestObject15()));
    ...
    public void test_missing_notFirst() {
        Config cfg = JsoniterSpi.getCurrentConfig().copyReBuilder()
            .indentionStep(2)
            .encodingMode(EncodingMode.DYNAMIC_MODE)
            .build();
        assertEquals("{\n" +
