...
class IterImplForStreaming {
	...
	private static final String CON = "readObjectFieldAsHash";
    ...
    public static final int readObjectFieldAsHash(JsonIterator iter) throws IOException {
        if (nextToken(iter) != '"') {
            throw iter.reportError(CON, "expect \"");
        }
        ...
        for (; ; ) {
            ...
            if (!loadMore(iter)) {
                throw iter.reportError(CON, "unmatched quote");
            }
        }
    }
    ...
    static final void skipArray(JsonIterator iter) throws IOException {
        int level = 1;
        for (; ; ) {
            for (int j = iter.head; j < iter.tail; j++) {
                switch (iter.buf[j]) {
    ...
    static final byte readByte(JsonIterator iter) throws IOException {
        if (iter.head == iter.tail && !loadMore(iter)) {
    ...
    private static void path2(int bc, JsonIterator iter, int j, int u2) throws IOException {
    ...
    private static void path(int bc, JsonIterator iter, int j) {
    ...
    private static void slow(int bc) {
