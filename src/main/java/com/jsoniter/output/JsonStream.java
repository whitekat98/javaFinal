...
public class JsonStream extends OutputStream {
    ...
    public final void writeRaw(String val, int remaining) throws IOException {
        if (out == null) {
            ...
            byte[] tempBytes = val.substring(0, remaining).getBytes();
            System.arraycopy(tempBytes, 0, buf, count, tempBytes.length);
            count += remaining;
            return;
        }
        int i = 0;
    ...
    public final <T> void writeVal(TypeLiteral typeLiteral, T obj) throws IOException {
        if (null == obj) {
            writeNull();
        } else {
    ...
    public static void setMode(EncodingMode mode) {
        Config newConfig = JsoniterSpi.getDefaultConfig().copyReBuilder().encodingMode(mode).build();
    ...
    public static void setIndentionStep(int indentionStep) {
        Config newConfig = JsoniterSpi.getDefaultConfig().copyReBuilder().indentionStep(indentionStep).build();
        JsoniterSpi.setDefaultConfig(newConfig);
        JsoniterSpi.setCurrentConfig(newConfig);
    }
