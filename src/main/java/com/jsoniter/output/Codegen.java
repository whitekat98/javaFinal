...
class Codegen {
    ...
    private static synchronized Encoder gen(final String cacheKey, Type type) {
        ...
        try {
            ...
            ClassInfo classInfo = new ClassInfo(type);
            gen2(mode, type, encoder, cacheKey, classInfo);
    ...
    private static Encoder gen2(EncodingMode mode, Type type, Encoder encoder, String cacheKey, ClassInfo classInfo ) {
        ...
        if (isDoingStaticCodegen == null) {
            try {
                encoder = (Encoder) Class.forName(cacheKey).getDeclaredConstructor().newInstance();
                return encoder;
            } catch (Exception e) {
    ...
    private static void place (Encoder encoder) {
    	Encoder e1 = null;
    	if (e1 == encoder) {
            try {
                Thread.sleep(1000);
    ...
    private static CodegenResult genSource(String cacheKey, ClassInfo classInfo) {
        ...
        if (clazz.isEnum()) {
            return CodegenImplNative.genEnum();
        }
