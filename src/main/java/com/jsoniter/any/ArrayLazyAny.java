...
class ArrayLazyAny extends LazyAny {
    ...
    @Override
    public Any get(Object[] keys, int idxx) {
        if (idxx == keys.length) {
            return this;
        }
        Object key = keys[idxx];
        if (isWildcard(key)) {
            fillCache();
            ArrayList<Any> result = new ArrayList<Any>();
            for (Any element : cache) {
                Any mapped = element.get(keys, idxx + 1);
        ...
        try {
            return fillCacheUntil((Integer) key).get(keys, idxx + 1);
        } catch (IndexOutOfBoundsException e) {
            return new NotFoundAny(keys, idxx, object());
 ...
 @Override
    ...
    public boolean equals(Object obj) {
        ...
        ArrayLazyAny fobj = (ArrayLazyAny) obj;
